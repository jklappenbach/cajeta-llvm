...
; A 2-D storage image write: llvm.spv.resource.store.2d takes a 2-component
; integer coordinate (unlike store.typedbuffer, which takes a scalar index) and
; lowers to a single OpImageWrite. The image declares the R32f known format, so
; the write needs only the Shader capability (Unknown format would require
; StorageImageWriteWithoutFormat, which is unavailable for SPIR-V < 1.6).
